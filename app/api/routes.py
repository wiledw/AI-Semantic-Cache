...
def _increment_stat(redis_client: Redis, key: str) -> None:
    try:
...
def _get_stat(redis_client: Redis, key: str) -> int:
    try:
...
def query_endpoint(payload: QueryRequest) -> QueryResponse:
    ...
    _increment_stat(redis_client, "stat:requests")

    try:
        embedding = cache.get_or_create_embedding(query_text)
    ...
    if not payload.forceRefresh:
        ...
        entry, similarity = cache.find_similar(embedding, threshold=threshold)
        if entry:
            logger.info("Cache hit with similarity %.4f", similarity)
            _increment_stat(redis_client, "stat:cache_hits")
            return QueryResponse(
                response=entry["response"],
                metadata=ResponseMetadata(source="cache", similarity=similarity),
            )
        logger.info("Cache miss. Best similarity %.4f", similarity)
        _increment_stat(redis_client, "stat:cache_misses")
    else:
        _increment_stat(redis_client, "stat:cache_misses")

    try:
        answer, used_llm = openai_client.get_completion(query_text)
    except Exception as exc:
        logger.exception("LLM call failed: %s", exc)
        raise HTTPException(status_code=502, detail="LLM service failure.")

    if used_llm:
        cache.store_response(query_text, embedding, answer, ttl_seconds)
    else:
        _increment_stat(redis_client, "stat:llm_fallbacks")
...
def stats_endpoint() -> StatsResponse:
    ...
    llm_calls = _get_stat(redis_client, "llm_call_count")
    llm_fallbacks = _get_stat(redis_client, "stat:llm_fallbacks")
